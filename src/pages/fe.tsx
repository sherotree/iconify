...
export default function Home() {
  ...
  useEffect(() => {
    fetch("https://api.iconify.design/collection?prefix=fe&info=true&chars=true&aliases=true")
      .then((res) => res.json())
      .then((data) => {
        setData(data);
      });
  ...
  return (
    <main className={`flex min-h-screen flex-col items-center p-24 gap-6 ${inter.className}`}>
      ...
      <div>
        <div className="font-bold mb-3">uncategorized</div>
        <div className="flex gap-3 flex-wrap w-[600px]">
          {(data?.uncategorized ?? []).map((category: any) => (
            <div
              key={category}
              title={category}
              onClick={() => {
                parent.postMessage({ pluginMessage: category, pluginId: "777777" }, "*");
              }}
            >
              <Image
                src={`https://api.iconify.design/${data?.prefix}/${category}.svg`}
                width={data?.info?.height}
